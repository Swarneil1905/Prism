...
function VerdictBadge({ verdict, override }: { verdict: string; override?: string | null }) {
  ...
  return (
    <span style={{
      ...
      {pass ? "Pass" : "Fail"}
      {override && <span style={{ opacity: 0.55, fontWeight: 400, fontSize: 10 }}>(override)</span>}
    </span>
  )
}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 20 }}>
        ...
        <div style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
          ...
          <div style={{
            ...
            height: 36, alignItems: "center", padding: "0 16px", gap: 12,
            background: "var(--surface-raised)",
          }}>
            {["Trace", "Score", "Verdict", "Actions"].map(h => (
              <div key={h} style={{ fontSize: 10, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-3)" }}>{h}</div>
            ))}
          </div>
          ...
          {evals.map(ev => (
            <div key={ev.id}>
              ...
              <div
                ...
                style={{
                  display: "grid", gridTemplateColumns: "100px 1fr 140px 120px",
                  minHeight: 44, alignItems: "center", padding: "8px 16px", gap: 12,
                ...
                <div style={{ display: "flex", gap: 6 }} onClick={e => e.stopPropagation()}>
                  <button
                    ...
                    style={{
                      ...
                      border: "1px solid rgba(16,185,129,0.3)", color: "var(--green)", background: "none",
                  ...
                  <button
                    ...
                    style={{
                      height: 24, padding: "0 8px", fontSize: 11, fontWeight: 500, borderRadius: 4,
                      border: "1px solid rgba(239,68,68,0.3)", color: "var(--red)", background: "none",
                      cursor: "pointer", opacity: overriding === ev.id ? 0.5 : 1,
                    }}
                  >Fail</button>
                </div>
              ...
              {expanded === ev.id && (
                <div style={{
                  ...
                  <div style={{ fontSize: 10, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-3)", marginBottom: 6 }}>
                    Judge Reasoning
                  </div>
                  <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {stats && (
            <div style={{
              ...
              <div style={{ fontSize: 10, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-3)", marginBottom: 12 }}>
                7-Day Pass Rate
              </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <div style={{ fontSize: 12, color: "var(--green)" }}>
                <span style={{ fontWeight: 600 }}>{passCount}</span>{" "}
                <span style={{ color: "var(--text-3)", fontWeight: 400 }}>pass</span>
              </div>
              <div style={{ fontSize: 12, color: "var(--red)" }}>
                <span style={{ fontWeight: 600 }}>{failCount}</span>{" "}
                <span style={{ color: "var(--text-3)", fontWeight: 400 }}>fail</span>
              </div>
            </div>
          </div>
          ...
          {/* Info */}
          <div style={{
            background: "var(--indigo-muted)", borderRadius: 8, padding: "12px 14px",
            border: "1px solid rgba(99,102,241,0.2)",
          }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: "var(--indigo)", marginBottom: 5 }}>How evals work</div>
